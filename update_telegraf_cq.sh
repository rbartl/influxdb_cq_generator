...
curl -s -XPOST -G 'http://localhost:8086/query?pretty=true'  --data-urlencode "q=CREATE RETENTION POLICY \"default\" ON \"$DESTDB\" DURATION 52w REPLICATION 1 DEFAULT" > /dev/null

set -e

MEASUREMENTS=$(curl  -s -H "Accept: application/csv" -XPOST -G 'http://localhost:8086/query?pretty=true'  --data-urlencode "q=SHOW MEASUREMENTS on $DB" | sed s/.*,// | tail -n +2 )

for M in $MEASUREMENTS ; do
 VALUES=$( curl -s -H "Accept: application/csv" -XPOST -G 'http://localhost:8086/query?pretty=true'  --data-urlencode "q=SHOW FIELD KEYS on $DB from $M" | grep -v ,string | sed s/.*,,// | sed s/,.*//  | tail -n +2 )
 VALUESSETTER=""
 for V in $VALUES ; do
